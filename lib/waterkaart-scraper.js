...
class WaterkaartScraper {
  ...
  async getLocationDetail(urlName) {
    try {
      ...
      // Temperature: <span class="meet-waarde">~13.8</span> <span class="meet-eenheid">°C</span>
      let temperature = null;
      let measurementTime = '';
      const tempMatch = html.match(/~([\d.]+)<\/span>\s*<span[^>]*>°C/);
      if (tempMatch) temperature = parseFloat(tempMatch[1]);

      // Measurement time: <span style="...">Temp: 22-04-2026, 13:10</span>
      const timeMatch = html.match(/Temp:\s*([\d-]+,\s*[\d:]+)/);
      if (timeMatch) measurementTime = timeMatch[1].trim();

      // Quality: "zwemwaterkwaliteit bij X is <span class="meet-waarde bold groen">goed</span>"
      let quality = '';
      const qualityMatch = html.match(/zwemwaterkwaliteit[^<]*is\s*<span[^>]*>([^<]+)<\/span>/i);
